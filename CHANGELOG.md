...
- **IMAGE sample, color, and memory-layout vocabulary.** Open `SampleLayout` values cover packed, indexed, alpha, luma, planar YUV, and interleaved YUV surfaces. `SurfaceDescriptor` reads and writes the canonical 32-byte logical surface record while deriving plane geometry and minimum stride without allocation. `PlaneMemoryLayout` validates allocation extent, stride, DATA offset, byte length, and power-of-two address alignment in a nonredundant 24-byte physical record. Indexed color tables remain separate from sample planes, and YUV color descriptions require explicit matrix, range, and chroma siting.
