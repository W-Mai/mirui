...
- **Sectioned FRAMES authoring.** `FramesEncoder` measures complete RAW, RLE, native-pixel, LZ4, omitted and previous-frame residual storage, writes aligned shared DATA and coding records, indexes independent recovery frames, and returns deterministic per-frame decisions. Tight packed, indexed and YUV input sizes reuse derived plane strides.
