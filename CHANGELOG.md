...
- **Checked FRAMES containers and generation.** `Reader`, `Document`, and mutable chunk handles share the sectioned FRAMES contract. Document output places declared input-aligned DATA at valid file addresses, while `cargo xtask gen-mirx frames` converts ordered source images, reports selected representations, and reopens the completed container before writing it.
