...
- **Animated curved text.** Fixed-range baselines invalidate visual geometry without rerunning intrinsic layout, stale path revisions replace their cache entries, curve measurement retains a stable minimum subdivision topology across morph frames, tangents interpolate continuously, the embedded showcase moves text through a reactive path offset without mutating its curve, and MIRX font metrics avoid repeated representation selection.
