...
- **MIRX image generation.** `cargo xtask gen-mirx image` converts common image inputs through ICU with explicit sample format, RAW/native-pixel/RLE/LZ4/frequency coding, quantized quality and RAW row alignment. Generated bytes are reopened and profile-preflighted through `mirx::Reader` before being written.
