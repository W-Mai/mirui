...
- **Deployable MIRX frame reports.** `cargo xtask gen-mirx frames` accepts output base, plane, width, height, and stride constraints plus finite or unbounded play count. Its report includes coding choices, loss policy, compression ratio, recovery and input-alignment bounds, mirui compatibility, exact playback buffers, group slots, and physical plane layouts.
