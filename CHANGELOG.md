...
- **MIRX image generation.** `cargo xtask gen-mirx image` converts common image inputs through ICU with explicit sample format, RAW/native-pixel/RLE/LZ4 coding and row alignment. Generated bytes are reopened through `mirx::Reader` before being written.
