...
- **Gallery game covers.** The complete preview surface now opens its game while preserving the arrow affordance and independent source action.
...
- **Reactive custom-view rendering.** `render_key: ${ ... }` declares the reactive identity of custom drawing and repaints the widget without forcing layout or exposing manual effect wiring in composition code.
...
- **Gallery gesture and glass studies.** Pinch + Rotate uses a themed live-transform target with compact status telemetry, while the fixed-size glass demo keeps its blur sample legible at native pixel density.
...
- **Stable MIRX geometry costs.** VECTOR glyph and projective-matrix wire sizes come from their owning types, while Typography Lab describes projective fallback as caller-budgeted instead of presenting one demo allocation as a format limit.
...
- **Kinetic Type gallery demo.** Three retained cubic baselines morph continuously through signal-bound path revisions, with mixed-script curved text and interactive amplitude, speed, direction, and braking controls. The 128 × 128 embedded profile streams a repeated bitmap glyph run over a fixed wave with bounded runtime storage.
...
- **Physical render regions.** `PhysicalRect` carries clipped integer pixels across surface flush, readback, mirror, scroll, and fallback boundaries; exact fallback storage supports explicit address and row-stride alignment.
...
- **WGPU deferred draw failures.** Checked image and glyph draws report source upload, atlas decoding, uniform, and instance-buffer failures instead of silently dropping the affected command.
...
- **Text filter diagnostics.** A scene needing unresolved glyph ink bounds reports a missing rendering capability instead of blaming valid text geometry.
- **Borrowed plane uploads.** Textures borrowed from mutable aligned planes bypass pointer-keyed GPU upload caches so changed bytes are not mistaken for unchanged content.
- **Transparent projective fallback.** Software source-over keeps straight RGBA on transparent targets, and projected fallback preserves the target alpha instead of forcing modified pixels opaque.
...
- **Canonical text layout workspace.** Mirui delegates bidi resolution, shaping, line breaking and private scratch growth to `textflow-rs` while retaining caller-owned final output, stable layout handles and one shared cache-memory budget.
...
- **Native posed 2D text.** SDL GPU batches consecutive coverage glyphs against cached scalar atlases, while Web Canvas applies each canonical glyph frame through its local transform stack.
...
- **Canonical MIRX reader support paths.** Reader options, limits, findings, iterators, and payload validation diagnostics live under `mirx::reader`; duplicate crate-root exports have been removed.
...
- **Namespaced MIRX source policies.** `extension::SourcePolicy` grants rewrite capabilities to matching source chunks through `OpenOptions::with_source_policies`.
- **Lean MIRX removal contract.** `Document::remove` returns success without duplicating descriptor state; `remove_to_vec` remains the payload-recovery operation.
- **Explicit MIRX typed edit guards.** `edit_font`, `edit_vector`, `edit_meta`, and `edit_palette` return owned working values; `commit()` applies the validated replacement and dropping a guard discards it.
...
- **Explicit MIRX decode memory contracts.** `DecodeRequest` separates reconstruction, compute, and direct-upload intent from CPU, memory-mapped Flash, coherent shared, non-coherent shared, and device-only placement. Image and frame playback plans report encoded-address alignment, output, reusable workspace and restore-snapshot alignment, and required cache synchronization while rejecting unsupported execution or inaccessible buffers before preflight. mirui texture and frame adapters retain the same contract; managed texture loading accepts only CPU-owned output and workspace.
- **Preflighted MIRX frame playback.** `FramesPlaybackPlan` validates every frame representation and reports exact reusable group, canvas, codec-workspace, and disposal-backup storage. mirui's `MirxFramesPlan` binds those caller-owned buffers once and presents each requested frame as a borrowed render texture while preserving bounded temporal state.
- **Deployable MIRX frame reports.** `cargo xtask gen-mirx frames` accepts input/output/workspace placement, output base, plane, width, height and stride constraints, workspace alignment, and finite or unbounded play count. Its report includes coding choices, loss policy, compression ratio, recovery and encoded-address bounds, cache actions, mirui compatibility, exact playback buffers, group slots, and physical plane layouts.
- **MIRX frame timeline lookup.** `FrameTimeline::locate` maps absolute ticks to frame, play ordinal, and in-frame offset across canonical variable durations. Finite timelines use an exclusive end boundary; unbounded timelines retain `u64` play ordinals. mirui sessions expose the same lookup through `present_at` without an allocated cumulative table.
- **Executable MIRX frame playback example.** `mirx_frames_snapshot` builds a compressed variable-duration sequence, binds fixed 64-byte-aligned playback storage, resolves presentation ticks, and writes a three-frame contact sheet from one reused canvas.
...
- **Portable frame-residual execution.** Validated FrameDelta blocks dispatch through a public allocation-free kernel seam. A target-gated `wide::u8x16` implementation accelerates eligible literal, repeated, and patterned residuals on x86-64, AArch64, and wasm32 from one source path. Plan-time short-stream selection and scalar tails preserve compact execution and bit-exact modulo-256 reconstruction; unsupported targets do not include the vector dependency.
...
- **Sectioned FRAMES authoring.** `FramesEncoder` measures whole-frame and sparse-tile RAW, RLE, native-pixel, LZ4, frequency and previous-frame residual storage, chooses compact cell and byte-range indexes, writes aligned shared DATA and coding records, binds indexed color tables, stores per-frame duration overrides, indexes independent recovery frames, and returns deterministic per-frame decisions. Tight packed, indexed and YUV input sizes reuse derived plane strides.
- **Checked FRAMES containers and generation.** `FramesView`, `FramesAsset`, and `FramesError` expose the sectioned format without a parallel atlas container API. `Reader`, `Document`, and mutable chunk handles share that contract. Document output places declared input-aligned DATA at valid file addresses, while `cargo xtask gen-mirx frames` converts ordered source images, reports selected representations, and reopens the completed container before writing it.
- **Frequency-coded frame candidates.** Reversible and explicitly admitted quantized frequency profiles participate in whole-frame selection across packed color and YUV planes. Quantized frames are reconstructed into authoring history before later omission or residual decisions, keeping encoder and decoder predictors identical.
...
- **MIRX image generation.** `cargo xtask gen-mirx image` converts common image inputs through ICU with explicit sample format, RAW/native-pixel/RLE/LZ4/frequency coding, quantized quality and RAW row alignment. Generated bytes are reopened and profile-preflighted through `mirx::Reader`; deployment reports use a shared decode-memory contract for physical output geometry, buffer placement, cache actions, workspace alignment, and stored versus actual input address alignment.
...
- **Fixed-point raw storage is private.** `Fixed` and `Fixed64` use semantic constructors, comparisons, and sign predicates. MIRX wire codecs use explicit little-endian byte conversion, and cross-crate conversion preserves every fixed-point bit without exposing raw integer construction to applications. CHA rejects raw fixed APIs and tuple-field access outside an exact internal storage allowlist.
- **MIRX alignment values are validated at construction.** `ByteAlignment` replaces unvalidated address-alignment integers across IMAGE, FONT, FRAMES, decode requests, memory plans, media records, and mirui adapters. Zero and non-power-of-two values cannot enter later planning or encoding stages.
- **FRAMES playback binds named storage.** `FramesPlaybackPlan::bind(PlaybackStorage { .. })` validates group slots, canvas, codec workspace, and disposal backup before creating a session. `FramesView` exposes `group_workspace_len`; direct session shortcuts are removed, and low-level frame selection records remain under `payload::frames` instead of the crate root.
- **IMAGE unit-index naming is explicit.** `EncodedImageAsset::with_unit_index` and `unit_index` distinguish compact per-unit ranges from unrelated collection indexes.
