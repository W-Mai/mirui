...
`gen-mirx image` converts common images through ICU and writes RAW, native pixel, RLE, or LZ4 IMAGE storage. The generated bytes use the same checked profiles consumed by `mirx::Reader` and mirui's texture loader.

```shell
cargo xtask gen-mirx image --in logo.png --out logo.mirx --format rgba8888 --coding lz4 --stride-align 64
```
