...
`DecodeRequest` adds execution and memory policy without mixing it into surface geometry. The default reconstructs into CPU memory; advanced plans distinguish memory-mapped Flash, coherent shared memory, non-coherent shared memory, and device-only storage, with independent workspace alignment and directional cache-sync reporting. The built-in decoder rejects compute, direct-upload, and CPU-inaccessible buffers before image preflight instead of pretending that a slice-backed decode can satisfy them. See [decode execution and memory](docs/decode-memory.md).
