...
`media::UnitIndex` resolves DATA-relative byte ranges without per-unit geometry. Fixed-size units need no table; variable-size units use `u32` offsets or `u16` lengths with one `u32` checkpoint per 64 units. Sequential iteration is constant-time per range; checkpointed random lookup sums at most 63 preceding lengths. `UnitIndexEncoding` validates and writes caller-supplied lengths without allocation, silently changing representations, or touching output on error. A byte index establishes neither independent decodability nor valid image coverage.
...
`coding::Lz4` preflights and decodes independent raw LZ4 blocks with an exact caller-supplied decoded length. Backward references stay within the block and use caller output as history; no frame, size prefix, external dictionary or allocation is required. Errors preserve output, and successful decoding retains the suffix. See [LZ4 coding](docs/lz4-coding.md).

`DecodeUnitRef::decode_plan` combines supported profile preflight with `UnitMemoryPlan`. PIXEL and RLE units decode directly into planned rows, including padded strides and allocation extents, without a tight staging buffer. RLE supports selected packed, indexed, alpha and YUV planes; tight rows are consumed in original plane-index order, even when an element crosses rows or planes. Actual output address and capacity are checked before writes; padding and unused sub-byte row bits become zero, while suffix bytes stay unchanged. `DecodedUnit` borrows only the destination and exposes `SurfacePlane` row access, while its memory plan retains original source regions. Verify source integrity with `ImageGroups::validate_unit` before executing an encoded IMAGE unit. Unsupported coding or frame references fail during planning.
