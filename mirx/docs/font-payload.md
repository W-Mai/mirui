...
`ADVANCES` contains `N` signed 24.8 values in face design units. `SHAPING` contains a bounded SFNT subset with cmap, horizontal metrics, shaping tables, and outlines; it replaces `ADVANCES` when runtime shaping owns placement. Storing both is invalid.
