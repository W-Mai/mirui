...
`FramesAsset::new(sequence, surface, groups, frame_group_counts)` is the borrowed authoring path for already encoded units. Each `UnitGroup` owns its coding profile, selection, ranges, reference rule and DATA slice. The encoder deduplicates profiles, chooses canonical compact tables and lays out aligned DATA; callers do not construct group records or combined index bytes.
...
`Document::push_frames(encoded)` transfers the finished payload without cloning it. The document writer reads its declared group input alignment and chooses a chunk position that makes the final file-relative DATA addresses valid. `Document::get(id)?.frames()` and `ChunkRef::frames(limits)` return the same `FramesView`; both retain fixed file placement when the payload came from an encoded container.
