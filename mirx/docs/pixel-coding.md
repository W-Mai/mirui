...
`DecodeUnitRef::decode_plan(requirements)` derives the exact pixel count from the unit's logical plane geometry and checks its profile before returning `UnitDecodePlan`. `decode_into` writes runs directly into the planned stride, splitting a run at row boundaries when needed. No tight image staging allocation occurs. Destination address and capacity are validated first; allocation-only rows, pixels and stride padding become zero, and the output suffix is preserved.

`DecodedUnit` borrows only output and can outlive the encoded input. `plane(0)` exposes a decoded `SurfacePlane`; `memory_plan().plane(0).source_region()` retains placement in the original surface. This operation fills independent caller-owned unit storage, not an in-place region of a full-surface buffer. The scalar decoder reads input bytewise; a stored input-alignment declaration does not become an output requirement. Use media integrity validation separately before interpreting a unit's samples.
