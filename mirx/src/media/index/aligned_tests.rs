    ...
    for count in [0, 1, 63, 64, 65, 129, 4097] {
        for alignment in [1, 2, 64, 4096] {
            for encoding in [UnitIndexEncoding::Lengths16, UnitIndexEncoding::Lengths32] {
                ...
                let mut end = 0u64;
                for &len in &lengths {
                    let start = end.div_ceil(u64::from(alignment)) * u64::from(alignment);
...
fn fixed_gaps_and_last_u32_end_do_not_require_a_trailing_padding_span() {
    let fixed = UnitIndex::fixed(3, 5, 64).unwrap();
    assert_eq!(fixed.byte_len(), 133);
    assert!(fixed.iter().eq([0..5, 64..69, 128..133]));
    assert!(fixed.iter().rev().eq([128..133, 64..69, 0..5]));
    assert!(UnitIndex::fixed(0, u32::MAX, 1 << 31).unwrap().is_empty());
    let last = UnitIndex::fixed(1, u32::MAX, 64).unwrap();
    assert_eq!(last.iter().next(), Some(0..u32::MAX));
    assert_eq!(last.iter().next_back(), Some(0..u32::MAX));
    let separated = UnitIndex::fixed(2, 1, 1 << 31).unwrap();
    assert!(separated.iter().rev().eq([(1 << 31)..(1 << 31) + 1, 0..1]));
    assert_eq!(
        UnitIndex::fixed(3, 1, 1 << 31),
        Err(UnitIndexError::SizeOverflow)
    );
    let mut bytes = [0; 8];
    UnitIndexEncoding::Lengths32
        .encode_into(&[u32::MAX], 64, &mut bytes)
        .unwrap();
    let last = UnitIndex::lengths32(1, &bytes, 64).unwrap();
    assert_eq!(last.iter().next(), Some(0..u32::MAX));
    assert_eq!(last.iter().next_back(), Some(0..u32::MAX));
}
...
fn alignment_width_overflow_and_noncanonical_checkpoints_are_atomic_errors() {
    ...
    for alignment in [0, 3, u32::MAX] {
        assert!(matches!(
            UnitIndex::fixed(0, 0, alignment),
            Err(UnitIndexError::InvalidAlignment(_))
        ));
        assert!(UnitIndex::lengths16(0, &[], alignment).is_err());
        assert!(
            UnitIndexEncoding::Lengths32
                .encode_into(&[1], alignment, &mut output)
                .is_err()
        );
    }
    assert!(matches!(
        UnitIndexEncoding::Offsets.encode_into(&[3, 4], 64, &mut output),
    ...
    assert!(
        UnitIndexEncoding::Lengths16
            .encode_into(&[65536], 64, &mut output)
            .is_err()
    );
    assert!(
        UnitIndexEncoding::Lengths32
            .encode_into(&[u32::MAX, 0], 64, &mut output)
            .is_err()
    );
    assert!(
        UnitIndexEncoding::Lengths32
            .encode_into(&[1; 65], 64, &mut output[..267])
            .is_err()
    );
    assert_eq!(output, [0xad; 272]);
    let len = UnitIndexEncoding::Lengths32
        .encode_into(&[1; 65], 64, &mut output)
        .unwrap();
    assert_eq!(read_u32_le(&output, 4), Some(4096));
    assert!(UnitIndex::lengths32(65, &output[..len], 1).is_err());
    output[4] ^= 1;
    assert!(matches!(
        UnitIndex::lengths32(65, &output[..len], 64),
